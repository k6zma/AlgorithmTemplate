...
void findAns(int k, int s, std::vector<int> &ans, const std::vector<std::vector<int>> &A, const std::vector<int> &w, const std::vector<int> &p, int &total_weight, int &total_cost) {
    if (A[k][s] == 0) {
        return;
    }
...
int main() {
    ...
    for (int i = 1; i <= N; ++i) {
        for (int c = 0; c <= W; ++c) {
            A[i][c] = A[i - 1][c];
            if (c >= w[i]) {
                A[i][c] = std::max(A[i][c], A[i][c - w[i]] + p[i]);
            }
        }
    }
